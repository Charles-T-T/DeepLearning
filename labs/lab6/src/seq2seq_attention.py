...
def get_batch(batch_size, length):
    ...
    encoder_X = [[char2index(char) for char in list(SampleString)] for SampleString in batch_examples]
...
class Seq2SeqModel(nn.Module):
    def __init__(self):
        ...
        self.embedding_layer = nn.Embedding(self.vocab_size,embedding_dim=EMBEDDING_LAYER_DIM)
        self.encoder = nn.GRU(
        ...
        self.decoder = nn.GRU(
            input_size = EMBEDDING_LAYER_DIM + GRU_HIDDEN_DIM,  #将根据attention weight计算得到的 context vector和当前输入字符的embedding拼接作为decoder输入
            hidden_size = GRU_HIDDEN_DIM,
            num_layers = 1,
            batch_first= True
        )
        self.linear = nn.Linear(GRU_HIDDEN_DIM,self.vocab_size)
        self.attention_W = nn.Linear(GRU_HIDDEN_DIM,GRU_HIDDEN_DIM,bias=False)#双线性模型attention中的W


    def forward(self,encoder_X,decoder_X):
        encoder_output,hidden_state = self.encoding(encoder_X)
        decoder_output,decoder_hidden = self.decoding(encoder_output,decoder_X,hidden_state)
    ...
    def encoding(self,encoder_X):
        encoder_embedding = self.embedding_layer(encoder_X) #(batch_size, sequence_length, embedding_size)
    ...
    def get_context_vector(self, encoder_out, hidden_state):
        ...
        weight = torch.bmm(self.attention_W(encoder_out), hidden_state.permute(1,2,0)) #( batch_size, sequence_length ,1 )
        ###################################################################
        #填空 对计算出的attention分数进行softmax归一化(要注意对哪一维度的值进行归一化)
        weight =     #(batch_size_sequence_length,1)
        ###################################################################
        context_vectors = torch.sum(weight * encoder_out,dim=1,keepdim=True) #(batch_size, 1, hidden_size)
    ...
    def decoding(self, encoder_out, decoder_X, hidden_state):
        ...
        all_outputs = torch.cat(all_outputs,dim=1) #(batch_size,sequence_length,hidden_size)
...
def train(model,optimizer,criterion):
    for step in range(5000):
        ...
        Y = Y.view(-1)
        loss,using_time = train_one_step(model,optimizer,criterion,encoder_X, Y ,decoder_X)
        if (step + 1) % 100 == 0:
    ...
    for one in range(test_num):
        ...
        with torch.no_grad():
            for i in range(SEQ_LENGTH):
                ...
                output,hidden_state = model.get_next_token(encoder_out,input_X,hidden_state)
                current_Index = output.tolist()[0]
                current_Char = index2char(current_Index)
                generate_sequence += current_Char
        print("Input: {}, Should get: {}, Model generate: {}  |The result is {}".format(batch_samples, original_sequence ,generate_sequence, original_sequence == generate_sequence ))
